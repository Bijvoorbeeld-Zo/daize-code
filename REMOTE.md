...
| ----------------------- | --------------------- | ---------------------------------- |
| `--mode <web\|desktop>` | `DAIZE_MODE`          | Runtime mode.                      |
| `--port <number>`       | `DAIZE_PORT`          | HTTP/WebSocket port.               |
| `--host <address>`      | `DAIZE_HOST`          | Bind interface/address.            |
| `--state-dir <path>`    | `DAIZE_STATE_DIR`     | State directory.                   |
| `--dev-url <url>`       | `VITE_DEV_SERVER_URL` | Dev web URL redirect/proxy target. |
| `--no-browser`          | `DAIZE_NO_BROWSER`    | Disable auto-open browser.         |
| `--auth-token <token>`  | `DAIZE_AUTH_TOKEN`    | WebSocket auth token.              |
