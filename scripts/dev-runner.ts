...
import { NetService } from "@daize/shared/Net";
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@daize/contracts",
    "--filter=@daize/web",
    "--filter=@daize.ai/cli",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=@daize.ai/cli"],
  "dev:web": ["run", "dev", "--filter=@daize/web"],
  "dev:desktop": ["run", "dev", "--filter=@daize/desktop", "--filter=@daize/web", "--parallel"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("DAIZE_PORT_OFFSET"),
  devInstance: optionalStringConfig("DAIZE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid DAIZE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `DAIZE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric DAIZE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed DAIZE_DEV_INSTANCE=${seed}` };
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...baseEnv,
      DAIZE_PORT: String(serverPort),
      PORT: String(webPort),
      ELECTRON_RENDERER_PORT: String(webPort),
      VITE_WS_URL: `ws://localhost:${serverPort}`,
      VITE_DEV_SERVER_URL: devUrl?.toString() ?? `http://localhost:${webPort}`,
      DAIZE_STATE_DIR: resolvedStateDir,
    };

    if (host !== undefined) {
      output.DAIZE_HOST = host;
    }

    if (authToken !== undefined) {
      output.DAIZE_AUTH_TOKEN = authToken;
    } else {
      delete output.DAIZE_AUTH_TOKEN;
    }

    if (noBrowser !== undefined) {
      output.DAIZE_NO_BROWSER = noBrowser ? "1" : "0";
    } else {
      delete output.DAIZE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.DAIZE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.DAIZE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.DAIZE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.DAIZE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.DAIZE_MODE = "web";
      delete output.DAIZE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read DAIZE_PORT_OFFSET/DAIZE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("DAIZE_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv("DAIZE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD"),
      logWebSocketEvents: readOptionalBooleanEnv("DAIZE_LOG_WS_EVENTS"),
    };
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.DAIZE_PORT)} webPort=${String(env.PORT)} stateDir=${String(env.DAIZE_STATE_DIR)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  stateDir: Flag.string("state-dir").pipe(
    Flag.withDescription("State directory path (forwards to DAIZE_STATE_DIR)."),
    Flag.withFallbackConfig(optionalStringConfig("DAIZE_STATE_DIR")),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to DAIZE_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("DAIZE_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to DAIZE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("DAIZE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to DAIZE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("DAIZE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to DAIZE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("DAIZE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to DAIZE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("DAIZE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to DAIZE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("DAIZE_PORT")),
  ),
