...
- Dev commands default `DAIZE_STATE_DIR` to `~/.t3/dev` to keep dev state isolated from desktop/prod state.
...
Set `DAIZE_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `DAIZE_DEV_INSTANCE`)
- Example: `DAIZE_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `DAIZE_PORT_OFFSET` to a numeric offset.
