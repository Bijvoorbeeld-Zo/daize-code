import { ProjectId, ThreadId } from "@daize/contracts";
...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.daize/worktrees/daize-mvp/daize-4e609bb8",
    );
    expect(result).toBe("daize-4e609bb8");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.daize\\worktrees\\daize-mvp\\daize-4e609bb8",
    );
    expect(result).toBe("daize-4e609bb8");
  });

  it("uses the final segment even when outside ~/.daize/worktrees", () => {
