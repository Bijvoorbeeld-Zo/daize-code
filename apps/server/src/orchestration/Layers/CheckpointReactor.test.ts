...
import type { ProviderKind, ProviderRuntimeEvent, ProviderSession } from "@daize/contracts";
import {
  CommandId,
...
function createGitRepository() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "daize-checkpoint-handler-"));
...
describe("CheckpointReactor", () => {
  ...
  it("continues processing runtime events after a single checkpoint runtime failure", async () => {
    const nonRepositorySessionCwd = fs.mkdtempSync(
      path.join(os.tmpdir(), "daize-checkpoint-runtime-non-repo-"),
    );
