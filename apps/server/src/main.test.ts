...
import type { OrchestrationReadModel } from "@daize/contracts";
...
const runCli = (
  ...
  env: Record<string, string> = { DAIZE_NO_BROWSER: "true" },
) => {
  const uniqueStateDir = `/tmp/daize-cli-state-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  return Command.runWith(t3Cli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            DAIZE_STATE_DIR: uniqueStateDir,
            ...env,
          },
        }),
      ),
...
it.layer(testLayer)("server CLI command", (it) => {
  it.effect("parses all CLI flags and wires scoped start/stop", () =>
    Effect.gen(function* () {
      ...
      assert.equal(resolvedConfig?.stateDir, "/tmp/daize-cli-state");
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        DAIZE_MODE: "desktop",
        DAIZE_PORT: "4999",
        DAIZE_HOST: "100.88.10.4",
        DAIZE_STATE_DIR: "/tmp/daize-env-state",
        VITE_DEV_SERVER_URL: "http://localhost:5173",
        DAIZE_NO_BROWSER: "true",
        DAIZE_AUTH_TOKEN: "env-token",
      });
      ...
      assert.equal(resolvedConfig?.stateDir, "/tmp/daize-env-state");
  ...
  it.effect("prefers --mode over DAIZE_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over DAIZE_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        DAIZE_NO_BROWSER: "false",
      });
  ...
  it.effect("supports CLI and env for bootstrap/log websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        DAIZE_MODE: "desktop",
        DAIZE_LOG_WS_EVENTS: "false",
        DAIZE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        DAIZE_NO_BROWSER: "true",
      });
